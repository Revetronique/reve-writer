...
フォルダ確認後、以下の優先順でフローを選択する:

| 条件 | フロー |
|---|---|
| 選定写真リスト（Notion）が記入済み（`id:...` 付き） | → STEP 4へ直接スキップ（再アップロード不要） |
| 選定写真リストが記入済み（ファイル名のみ） | → STEP 4へスキップ（アップロードのみ実行） |
| `_manifest.json` がフォルダ内に存在する | → `image-selector` を起動して STEP 2〜3 を代替 |
| それ以外 | → STEP 2（従来のファイル名マッチング）へ |

### STEP 0: image_indexer.py の実行（任意・manifest未生成時のみ）

`_manifest.json` が存在しない、または未索引ファイルがある場合にユーザーが事前実行を望むとき。
**差分のみ処理する**（索引済みファイルはスキップ）。

手順:
1. フォルダ内の画像ファイル一覧を取得（`ls`）
2. `_manifest.json` に未登録のファイルを差分抽出
3. 未索引ファイル数・ファイル名を表示してユーザーに確認
4. OK後に `image-indexer` エージェントを起動（`--files` で未索引ファイルのみ渡す）
...
分析が完了したら `_manifest.json` が生成される。その後 `image-selector` を起動する。
...
$results | ForEach-Object {
  ...
  [PSCustomObject]@{ file = $r.slug; id = $r.id; url = $r.media_details.sizes.large.source_url }
} | Format-Table -AutoSize
```

取得した `id` と `media_details.sizes.large.source_url`（large版URL）をGutenbergの `{{photo_id}}` / `{{photo_url}}` に埋め込む。
`large` サイズが存在しない場合（元画像が小さい等）は `source_url`（フルサイズ）にフォールバックする。
一部失敗した場合は該当箇所に `[★写真: アップロード失敗]` を挿入する。

画像POSTのみ403（Powered by SiteGuard Lite）になる場合、WPプラグインのSiteGuard Liteではなく
ConoHa WING側のサーバーレベルWAF（OSコマンドインジェクション誤検知）が原因のことがある。
詳細・切り分け方・対応策は `docs/design/wp-post-troubleshooting.md` の「4. 画像アップロードのみ403になる」を参照。
...
**複数ファイル一括変換（Windows / PowerShell）:**
```powershell
Add-Type -AssemblyName System.Drawing
Get-ChildItem *.HEIC | ForEach-Object {
    $img = [System.Drawing.Image]::FromFile($_.FullName)
    $outPath = $_.FullName -replace '\.HEIC$', '.jpg'
    $img.Save($outPath, [System.Drawing.Imaging.ImageFormat]::Jpeg)
    $img.Dispose()
}
```

- Windowsに `sips` 相当の標準コマンドはない
- 上記は `System.Drawing`（WIC経由）を使うため、Microsoft Store の「HEIF 画像拡張機能」が事前にインストールされている必要がある
- 未インストールの場合は `ffmpeg -i input.HEIC output.jpg` や ImageMagick（`magick input.HEIC output.jpg`）の利用を検討する
...
```
> Y:\Event\2026-06-makerfaire\ の写真を索引して
```

Claude Codeが:
1. フォルダ内の画像ファイル一覧を `ls` で取得
2. `_manifest.json` と照合して未索引ファイルを差分抽出
3. 「未索引 X 枚 / 索引済み Y 枚」を表示して確認
4. OK後に `image-indexer` エージェントを起動（未索引ファイルのみ `--files` で渡す）
5. 完了後に索引内容（ファイル名＋caption冒頭）を一覧表示

フォルダ全体を再解析したい場合は「`--force` で再索引して」と付け加える。
...
| `image-selector` | manifest.json＋下書きから挿入写真を選定 | フェーズ0（任意・事前準備） |
...
**フェーズ-1（任意）:** `image-indexer` は `_manifest.json` が存在しない、または未索引ファイルがある場合にユーザー確認後に使用する
**フェーズ0（任意）:** `image-selector` は写真フォルダに `_manifest.json` がある場合のみ使用。ない場合は従来のファイル名マッチングに進む
...
```
以下を実行せよ:
- manifest_path: {NASフォルダの _manifest.json フルパス}
- draft_text: {Notionから取得した下書き全文}
manifest.jsonを読み込み、下書きの各H2セクションに対して
最適な写真を1〜3枚ずつ選定して提案せよ。
```
