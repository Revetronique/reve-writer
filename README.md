...
CLAUDE.mdに定義されたパイプライン（Notion → Claude Code → WordPress）を動作させるための準備状況をまとめたドキュメント。
...
**写真命名規則:**
- 聖地巡礼: `spot-{スラッグ}-{連番2桁}.jpg`（例: `spot-uchi-harbor-01.jpg`）
- イベント・お出かけ: `{任意説明}-{連番2桁}.jpg`（例: `makerfaire-keyboard-01.jpg`）
...
`remoteControlAtStartup: true` は Claude Code起動時にRemote Controlブリッジを自動起動し、claude.aiから直接このセッションに指示を送れるようにする設定。不要な場合は `false` に変更するか、このキーを削除してよい。
...
└── docs/
    ...
    └── prompts/
        ...
        └── claude-md-event-module.md     # イベント記事仕様リファレンス
```
...
CLAUDE.md で定義された 3 つの専門エージェントで記事投稿フローを実行。複数記事は**フェーズ1を並列**、**フェーズ2を順次**実行。

### 3つのエージェント

| エージェント | 役割 | 実行タイミング |
|-----------|------|-------------|
| **photo-uploader** | NAS 写真 → WordPress Media API | フェーズ1（並列） |
...
- **CLAUDE.md**: 全機能の仕様定義（システムの真実のソース）
