...
実測（2026-07-16）により、記事本文は **5,000文字以内** を目安とする。
Gutenbergブロック形式＋メタ情報込みのJSON（約13KB）で問題なく投稿できることを確認済み。

| 目安 | 状態 |
|------|------|
| ~5,000文字 | 問題なし（実測確認済み） |
| 5,000〜10,000文字 | 余裕あり（単純テキストで実測済み） |
| 10,000文字超 | 未検証。必要であれば都度確認 |

> **補足:** 以前の2,500文字制限はSiteGuard Liteのブロックを原因と推定していたが、
> 実測では20,000文字超でも通過した。現在の上限は5,000文字（SEO・読みやすさ基準）。
